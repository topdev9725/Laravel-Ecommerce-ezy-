      ...
      $.ajax({
       ...
       success:function(data)
       {
        ...
        if(admin_loader == 1)
          ...
          {
            if($('#reload').length>0) location.reload();
            table.ajax.reload();
            $('.alert-success').show();
            $('.alert-success p').html(data);
        if(admin_loader == 1)
...
if(admin_loader == 1)
 ...
 // CATALOG OPTION ENDS
      ...
      $('#confirm-delete, #confirm-complete, #confirm-reject').on('show.bs.modal', function(e) {
          $(this).find('.btn-ok').attr('href', $(e.relatedTarget).data('href'));
      });

      $('#confirm-delete .btn-ok, #confirm-complete .btn-ok, #confirm-reject .btn-ok').on('click', function(e) {

...
class Affiliate
{
    ...
    private static function checkSupId($sup_id, &$sub_ids) {
        $sub_ids = [];
        $gs = Generalsetting::findOrFail(1); 
        $sub_users = User::where('sup_id', '=', $sup_id)->where('is_vendor', 0)->get();
        if($sub_users) {
            foreach($sub_users as $user) {
                $sub_ids[] = $user->id;
            }
        }
        return ($sub_users->count() < $gs->affiliate_members);
    }
    ...
    private static function setAffiliateNewUser($user_id) {
        ...
        $query = "SELECT * FROM users AS t1 WHERE t1.id<>".$user_id." AND t1.is_vendor=0 AND (SELECT count(*) FROM users AS t2 WHERE t2.is_vendor=0 AND t2.sup_id=t1.id)<".$gs->affiliate_members." and id <> 1 ORDER BY t1.sup_id ASC, t1.id ASC";
    ...
    public static function setSnpCashback($user_id, $vendor_id, $amount) {        
        $gs = Generalsetting::findOrFail(1);
        ...
        $n = sizeof($user_ids);
        return self::_checkSnpCashback($user_id, $user_ids, $vendor_id, $amount);
    }
    
    private static function _checkSnpCashback($owner, $user_ids, $vendor_id, $amount) {
        $n = sizeof($user_ids); $order_id = 0; $order_number = ''; $total_cashback = 0;
        $gs = Generalsetting::findOrFail(1);
        $snp_cashback_rate = User::where('id', $vendor_id)->first()->snpcashback;
        
        $cashback_amount = $amount * ($gs->affiliate_rate/100) * ($snp_cashback_rate/100) / $n;
        $total_cashback = $amount * ($snp_cashback_rate/100);
        $total_cashback = floor($total_cashback * 100) / 100;
        
        $from = User::findOrFail($owner);
        $cashback_amount = floor($cashback_amount * 100) / 100;
        foreach($user_ids as $id) {
            $user = User::findOrFail($id);
            if($user) {
                    $user->affilate_income += $cashback_amount;
                    $user->save();                
                // self::newTransaction($id, $cashback_amount, $owner);

                $notif = new NewNotification();                
                $notif->setCashbackNotification($user->id, $from->name, $cashback_amount);
    ...
    private static function _checkOrderCashback($owner, $user_ids, $orders, $is_cur_month) {
        $amount = 0; $n = sizeof($user_ids); $order_id = 0; $order_number = ''; $total_cashback = 0;
        $gs = Generalsetting::findOrFail(1);
        foreach($orders as $order) {
            $order_id = $order->id;
            ...
            foreach($cart->items as $product) {
                ...
                $total_cashback += $product['price'] * ($category->charging_fee/100);
            }
    ...
    public static function setAutodebitCashback($user_id, $amount, $category_id) {        
        $gs = Generalsetting::findOrFail(1);
        if($gs->is_affilate == 0) {
            return false;
        }
        ...
        $n = sizeof($user_ids);
        return self::_checkAutodebitOrderCashback($user_id, $user_ids, $amount, $category_id);
    }

    private static function _checkAutodebitOrderCashback($owner, $user_ids, $amount, $category_id) 
    {
        $n = sizeof($user_ids);
        $gs = Generalsetting::findOrFail(1);

        $autodebit_category_rate = AutodebitCategory::where('id', $category_id)->first()->charging_fee;
        
        $cashback_amount = $amount * ($gs->affiliate_rate/100) * ($autodebit_category_rate/100) / $n;
        $total_cashback = $amount * ($autodebit_category_rate/100);
        $total_cashback = floor($total_cashback * 100) / 100;
    ...
    public static function setAutodebitInsurranceCashback($user_id, $amount, $category_id) 
    {        
        $gs = Generalsetting::findOrFail(1);
        if($gs->is_affilate == 0) {
            return false;
        ...
        $n = sizeof($user_ids);
        return self::_checkAutodebitInsurranceOrderCashback($user_id, $user_ids, $amount, $category_id);
    }

    private static function _checkAutodebitInsurranceOrderCashback($owner, $user_ids, $amount, $category_id) {
        $n = sizeof($user_ids);
        $gs = Generalsetting::findOrFail(1);

...
class GeniusMailer
{
    ...
    public function sendAutoMail(array $mailData)
    {
        ...
        $damount = Product::convertPrice($mailData['damount']);
        $wbalance = Product::convertPrice($mailData['wbalance']);
        ...
        if (!empty($mailData['damount'])) {
            $body = preg_replace("/{deposit_amount}/", $damount ,$body);
          }
          if (!empty($mailData['wbalance'])) {
            $body = preg_replace("/{wallet_balance}/", $wbalance ,$body);
          }
        $body = preg_replace("/{website_title}/", $setup->title ,$body);

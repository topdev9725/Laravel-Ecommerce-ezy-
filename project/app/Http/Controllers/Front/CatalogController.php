...
class CatalogController extends Controller
{
    ...
    public function category(Request $request, $slug=null, $slug1=null, $slug2=null)
    {
      ...
      foreach($category_ids as $category_id) {
          $category = Category::where('status', '=', 1)->where('id', '=', $category_id)->get();
          if (count($category)) {
              array_push($data['product_categories'], Category::where('status', '=', 1)->where('id', '=', $category_id)->get());
          }
      }
    ...
    public function categorystoreshow(Request $request)               // ---------------Ezy
    {
      $category_id = $request->category_id;

      if ($category_id != 0) { //when cateogory button press
        $category_stores = User::whereIn('is_vendor', array(1,2))
                                ->where('online', '=', 1)
        ...
        $category_stores = User::whereIn('is_vendor', array(1,2))->where('online', '=', 1)->get();
        $category_info = '';
      }

      if($request->ajax()) {  //when show all button press
        return view('front.ajax-category-store-show',compact('category_stores', 'category_info'));
      }

      return view('front.category-store-show',compact('category_stores', 'category_info'));
    }
    
    public function globalsearch(Request $request)              //-------------- Ezy
    {
      $search = $request->search;
      $search_type = $request->search_type;
      Session::put('search_type', $search_type);
      if ($search_type == "product" || $search_type == "") {
        $global_search_products = Product::where('name', 'like', '%'.$search.'%')->get();
        return view('front.global-search-products', compact('global_search_products'));
      } else {
        $global_search_stores = User::where('shop_name', 'like', '%'.$search.'%')->get();
        return view('front.global-search-stores', compact('global_search_stores'));
      }
    }

    public function globalstoresearch(Request $request)           //--------- when store type search combo box press
    {
      $search = $request->search;
      $store_type = $request->store_type;
      Session::put('store_type', $store_type);
      if ($store_type == 'online') {
        $search_results = User::where('shop_name', 'like', '%'.$search.'%')->where('online', '=', 1)->get();
      } elseif ($store_type == 'nearbymerchant') {
        $search_results = User::where('shop_name', 'like', '%'.$search.'%')->where('nearby', '=', 1)->get();
      } else {
        $search_results = User::where('shop_name', 'like', '%'.$search.'%')->where('autodebit', '=', 1)->get();
      }
      return view('front.ajax-store-search', compact('search_results', 'store_type'));
    }

    public function globalproductsearch(Request $request)        //---------- when product search combo box press
    {
      $search = $request->search;
      $sort_type = $request->sort_type;
      Session::put('sort_type', $sort_type);
      if ($sort_type == 'date_desc') {
        $search_results = Product::where('name', 'like', '%'.$search.'%')->orderBy('id', 'DESC')->get();
      } elseif ($sort_type == 'date_asc') {
        $search_results = Product::where('name', 'like', '%'.$search.'%')->orderBy('id', 'ASC')->get();
      } elseif ($sort_type == 'price_asc') {
        $search_results = Product::where('name', 'like', '%'.$search.'%')->orderBy('price', 'ASC')->get();
      } else {
        $search_results = Product::where('name', 'like', '%'.$search.'%')->orderBy('price', 'DESC')->get();
      }

      return view('front.ajax-product-search', compact('search_results'));
    }
    ...
    public function product1(Request $request, $slug)    //this is for visit store button  ---Ezy
    {
        $store_type = $request->store_type;
        $this->code_image();
        $productt = Product::where('slug','=',$slug)->firstOrFail();
        ...
        return view('front.product',compact('productt','curr','vendors','store_type'));

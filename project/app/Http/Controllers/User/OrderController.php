...
class OrderController extends Controller
{
    ...
    {
        $user = Auth::guard('web')->user();
        $order_infos = $user->orders()->where('status', $order_status)->orderBy('created_at','DESC')->get();

        return view('user.order-status', compact('order_infos', 'order_status'));            
    }

    public function autodebitorder()
    {
        $user = Auth::guard('web')->user();
        $ordered_autodebits = AutodebitOrder::where('user_id', $user->id)->orderBy('created_at','DESC')->get();
        $ordered_insurrances = AutodebitInsurranceOrder::where('user_id', $user->id)->orderBy('created_at', 'DESC')->get();

        return view('user.autodebit-order', compact('ordered_autodebits', 'ordered_insurrances'));      
    }

...
class User extends Authenticatable
{
    ...
    protected $fillable = ['name','photo','state','zip','residency','city','country', 'address', 'phone', 'fax', 'email','password','affilate_code','verification_link','shop_name','owner_name','shop_number','shop_address','reg_number','shop_message','is_vendor','shop_details','shop_image','f_url','g_url','t_url','l_url','f_check','g_check','t_check','l_check','shipping_cost','date','mail_sent','balance', 'sup_id', 'latitude', 'longitude', 'shop_latitude', 'shop_longitude', 'shop_logo', 'opening_hours','online','nearby','autodebit', 'category_id', 'snpcashback'];
    ...
    {
        return $this->hasMany('App\Models\Deposit','user_id')->orderBy('id', 'desc')->get();
    }
    ...
    {
        return $this->hasMany('App\Models\NewNotification','user_id');
    } 
    ...
    public function get_new_notifications($is_read=-1)
    {
        if(count($this->new_notifications) > 0) {
            if($is_read==-1) {
                return $this->new_notifications();
            } else {
                return $this->new_notifications()->where('is_read','=', $is_read)->orderBy('id', 'desc')->get();
            }
        } else {
            return [];
        }        

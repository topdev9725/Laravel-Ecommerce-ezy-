...
@section('content')
					...
					<div class="content-area">
						<div class="mr-breadcrumb">
							<div class="row">
								<div class="col-lg-12">
										<h4 class="heading">{{ __('Autodebit Pending Orders') }}</h4>
										<ul class="links">
											<li>
												<a href="{{ route('vendor-dashboard') }}">{{ $langg->lang441 }} </a>
											</li>
											<li>
												<a href="{{ route('vendor-autodebit-order-pending') }}">{{ __('Autodebit Pending Orders') }}</a>
											</li>
										</ul>
								</div>
							</div>
...
{{-- DATA TABLE --}}
    ...
    <script type="text/javascript">
		$(document).on('change', '.vendor-btn', function(){
			...
			$('#confirm-delete2').find('.btn-ok').attr('href', $(this).val()+'?pending=1');
		});
		...
		var table = $('#geniustable').DataTable({
               ...
               processing: true,
               serverSide: true,
               ajax: '{{ route('vendor-autodebit-insurrrance-order-datatables') }}?pending=1',
               columns: [
						{ data: 'id', name: 'id' },
